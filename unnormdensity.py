...
#         x = vectors.IntVector(x)
#         out = stats.density(x, weights=weights, from=from_, to =to_, n=num)
#         y = np.array(out[1])
#         return_val = len(x)*y
#     elif len(weights)==1:
...
#         w = weights
#         wabs = [abs(p) for p in weights]
#         avgwabs = [abs(p) for p in wabs]
#         wpos = [max(0,w[i]) for i in range(len(w))]
#         wneg = [-min(0,w[i]) for i in range(len(w))]
#         # determine bandwidth using absolute masses
#         x1 = vectors.IntVector(x)
#         dabs = stats.density(x1, weights=avgwabs, from=from_, to =to_, n=num)
#         y1 = np.array(out[1])
#         bw = dabs["bw"]
#         # compute densities for positive and negative masses separately
#         sumwpos = sum(wpos)
#         sumwneg = sum(wneg)
#         avgwpos = [p/sumwpos for p in wpos]
#         avgwneg = [p/sumwneg for p in wneg]
#         outpos = stats.density(x1, bw=bw, adjust=1, weights=avgwpos, n=num)
#         outneg = stats.density(x1, bw=bw, adjust=1, weights=avgwneg, n=num)
#         # combine
#         return_val = list(np.subtract(sumwpos * np.array(outpos), sumwneg * np.array(outneg)))
    return return_val
...
def density(x, bw=None, adjust=1, kernel="gaussian",weights=None, n=512, from_=None, to_=None):
